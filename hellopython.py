import streamlit as st
import pandas as pd
from datetime import datetime
import requests

# --- 1. CẤU HÌNH HỆ THỐNG ---
SHEET_ID = "1UPyO04wZlOtHEIIGN_KHK11I6CslAI0pG2aH6CFECmA"
WEB_APP_URL = "https://script.google.com/macros/s/AKfycbwKjL7c8sDm-mJwKlUuOS1K4f5n6EW_AdaUzBI0WLFSE6pJbEwRnhGhugrs0qaIo6fDFg/exec"

def load_data(sheet_name):
    url = f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/gviz/tq?tqx=out:csv&sheet={sheet_name}"
    try:
        df = pd.read_csv(url)
        # Xóa khoảng trắng thừa ở tên cột để tránh lỗi KeyError
        df.columns = df.columns.str.strip()
        return df.fillna("")
    except Exception as e:
        st.error(f"Lỗi tải dữ liệu bảng {sheet_name}: {e}")
        return pd.DataFrame()

# --- 2. TỐI ƯU GIAO DIỆN (Clean & Clear) ---
st.set_page_config(page_title="Hệ thống Quản lý Văn bản UTT", layout="wide")

st.markdown("""
    <style>
    /* Màu xanh chủ đạo UTT */
    .main { background-color: #f8f9fa; }
    div.stButton > button {
        background-color: #004080; color: white; border-radius: 8px;
        width: 100%; font-weight: bold; border: none;
    }
    div.stButton > button:hover { border: 1px solid #004080; color: #004080; }
    .stDataFrame { background-color: white; border-radius: 10px; }
    h1, h2, h3 { color: #004080 !important; }
    </style>
    """, unsafe_allow_html=True)

# Khởi tạo trạng thái đăng nhập
if 'logged_in' not in st.session_state:
    st.session_state.logged_in = False

# --- 3. MÀN HÌNH ĐĂNG NHẬP / ĐĂNG KÝ ---
if not st.session_state.logged_in:
    st.title("🏫 HỆ THỐNG QUẢN LÝ VĂN BẢN UTT")
    tab1, tab2 = st.tabs(["🔐 ĐĂNG NHẬP", "📝 TẠO TÀI KHOẢN"])
    
    with tab1:
        u = st.text_input("Tên đăng nhập", key="l_u")
        p = st.text_input("Mật khẩu", type="password", key="l_p")
        if st.button("Vào hệ thống"):
            df_u = load_data("users")
            if not df_u.empty:
                # Ép kiểu string để so sánh chính xác
                user_match = df_u[(df_u['username'].astype(str) == u) & (df_u['password'].astype(str) == p)]
                if not user_match.empty:
                    st.session_state.logged_in = True
                    st.session_state.user_name = user_match.iloc[0]['fullname']
                    st.session_state.user_role = user_match.iloc[0]['role']
                    # Quan trọng: Ép kiểu ID thành chuỗi và xóa khoảng trắng
                    st.session_state.id_code = str(user_match.iloc[0]['id_code']).strip()
                    st.rerun()
                else: st.error("Sai tài khoản hoặc mật khẩu!")

    with tab2:
        st.subheader("Đăng ký thành viên mới")
        c1, c2 = st.columns(2)
        with c1:
            r_u = st.text_input("Username *", key="reg_u")
            r_f = st.text_input("Họ và Tên *", key="reg_f")
            r_id = st.text_input("Mã định danh (ID) *", key="reg_id")
        with c2:
            r_role = st.selectbox("Chức vụ *", ["Sinh viên", "Giảng viên", "Nhà trường"])
            r_c = st.text_input("Lớp *") if r_role == "Sinh viên" else "Cán bộ"
            r_p = st.text_input("Mật khẩu *", type="password", key="reg_p")
        
        if st.button("Xác nhận đăng ký"):
            if r_u and r_f and r_id and r_p:
                payload = {
                    "type": "register", "username": r_u, "password": r_p, 
                    "fullname": r_f, "id_code": r_id, "role": r_role, "class_name": r_c
                }
                requests.post(WEB_APP_URL, json=payload)
                st.success("🎉 Đăng ký thành công! Mời bạn quay lại tab Đăng nhập.")
            else: st.error("Vui lòng điền đủ thông tin!")

# --- 4. GIAO DIỆN CHÍNH (SAU ĐĂNG NHẬP) ---
else:
    st.sidebar.title(f"👤 {st.session_state.user_name}")
    st.sidebar.markdown(f"**Quyền:** {st.session_state.user_role}")
    st.sidebar.markdown(f"**Mã ID:** `{st.session_state.id_code}`")
    if st.sidebar.button("Đăng xuất"):
        st.session_state.logged_in = False
        st.rerun()

    menu = st.sidebar.radio("CHỨC NĂNG", ["📥 VĂN BẢN ĐẾN", "📤 SOẠN VĂN BẢN ĐI"])

    if menu == "📥 VĂN BẢN ĐẾN":
        st.header("📥 Hộp thư văn bản đến")
        df_in = load_data("doc_in")
        if not df_in.empty:
            # FIX LỖI NHẬN VĂN BẢN: Ép kiểu string và strip khoảng trắng cả 2 phía
            df_in['receiver_id'] = df_in['receiver_id'].astype(str).str.strip()
            df_mine = df_in[df_in['receiver_id'] == st.session_state.id_code]
            
            if not df_mine.empty:
                st.dataframe(df_mine, use_container_width=True)
            else:
                st.info(f"Hộp thư trống. (ID đang kiểm tra: {st.session_state.id_code})")
        else: st.info("Hộp thư hiện đang trống.")

    elif menu == "📤 SOẠN VĂN BẢN ĐI":
        st.header("📤 Soạn thảo văn bản đi")
        with st.form("form_send"):
            col_a, col_b = st.columns([2, 1])
            with col_a:
                target_id = st.text_input("Mã ID người nhận *", placeholder="Nhập MSSV/MSGV chính xác")
            with col_b:
                loai = st.selectbox("Loại văn bản", ["Công văn", "Thông báo", "Quyết định", "Đơn từ", "Khác"])
            
            note = st.text_input("Chú thích")
            content = st.text_area("Nội dung văn bản *", height=200)
            file = st.file_uploader("📎 Đính kèm tệp", type=["pdf", "png", "jpg", "docx"])
            
            if st.form_submit_button("🚀 GỬI VĂN BẢN"):
                if target_id and content:
                    payload = {
                        "type": "send_dual",
                        "date_sent": datetime.now().strftime("%d/%m/%Y %H:%M"),
                        "sender_name": st.session_state.user_name,
                        "sender_id": st.session_state.id_code,
                        "receiver_id": str(target_id).strip(),
                        "doc_type": loai, "note": note, "content": content,
                        "file_name": file.name if file else "Không có"
                    }
                    requests.post(WEB_APP_URL, json=payload)
                    st.success(f"✅ Đã gửi thành công tới ID {target_id}!")
                    st.balloons()
                else: st.error("Vui lòng nhập ID người nhận và nội dung!")